...
function requestBinary(requestOptions, filePath) {
  ...
  // Dynamically load node-fetch using require
  const fetchPromise = require('node-fetch').default;

  fetchPromise(requestOptions.url, requestOptions)
    .then((response) => {
      if (!response.ok) {
    ...
    .then((body) => {
      ...
      dest.on('finish', () => {
        ...
        console.log(`Received ${Math.floor(fs.statSync(filePath).size / 1024)}K total.`);
